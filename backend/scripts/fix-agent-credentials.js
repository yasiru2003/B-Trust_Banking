...
async function fixAgentCredentials() {
  const client = await pool.connect();
  
  try {
    console.log('🔧 Fixing agent credentials to match your existing setup...');
    console.log('=======================================================');
    
    // Fix AGENT001 to use your existing credentials
    console.log('1. Setting up AGENT001 with your existing credentials...');
    const agent001Password = 'password123';
    ...
    await client.query(
      ...
      [agent001PasswordHash, 'agent@bt.com', 'AGENT001']
    );
    ...
    // Setup AGENT002 with new credentials
    console.log('2. Setting up AGENT002 with new credentials...');
    ...
    if (agent001Result.rows.length > 0) {
      ...
      const isValidPassword = await bcrypt.compare('password123', agent001.password_hash);
      console.log(`✅ AGENT001 login test: ${isValidPassword ? 'SUCCESS' : 'FAILED'}`);
      console.log(`   Employee ID: ${agent001.employee_id}`);
    }
    ...
    if (agent002Result.rows.length > 0) {
      ...
      console.log(`   Employee ID: ${agent002.employee_id}`);
    }
    
    console.log('');
    console.log('🎉 Agent credentials fixed!');
    console.log('');
    ...
    console.error('❌ Error fixing agent credentials:', error.message);
  } finally {
    client.release();
    await pool.end();
    process.exit(0);
